...
class AccountMove(models.Model):
    ...
    sii_agency = fields.Many2one(
            comodel_name="aeat.sii.tax.agency",
            string="Agencia")

    @api.onchange("sii_agency")
    def onchange_sii_agency(self):
        for inv in self:
            if inv.sii_csv:
                return{
                        "warning":{
                            "message": _("No se puede modificar la agencia de un documento que ha sido registrado en el SII"),
                            }
                        }
    ...
    def _get_sii_taxes_map(self, codes):
        ...
        if self.sii_agency and self.sii_agency.agency_aeat_sii_map:
            sii_map = self.sii_agency.agency_aeat_sii_map
...
#            )
            ...
            raise exceptions.UserError("La agencia y su mapeo de impuestos deben estar definidos")
        tax_templates = (
    ...
    def _get_sii_header(self, tipo_comunicacion=False, cancellation=False):
        ...
        header = {
            "IDVersionSii": self.sii_agency.agency_sii_version,
            "Titular": {
    ...
    def _is_sii_type_breakdown_required(self, taxes_dict):
        ...
        self.ensure_one()
        _logger.error("TAXES_DICT IN BREAKDOWN REQUIRED: ")
        _logger.error(taxes_dict)
    ...
    def _get_sii_out_taxes(self):  # noqa: C901
        ...
        agency = self.sii_agency.get_external_id()[self.sii_agency.id].split('.')[1]

        for tax_line in tax_lines.values():
            tax = tax_line["tax"]
            _logger.error("IMPUESTO:")
            _logger.error(tax)
            breakdown_taxes = taxes_sfesb + taxes_sfesisp + taxes_sfens + taxes_sfesbe
            _logger.error(breakdown_taxes)
            ...
            if tax in (taxes_sfesb + taxes_sfesbe + taxes_sfesisp):
                ...
                if tax in taxes_sfesbe:
                    ...
                    if agency == 'aeat_sii_tax_agency_canary':                    
                        sub_dict.setdefault(
                            "NoExenta",
                            {
                        ...
                        sub_dict["NoExenta"]["DesgloseIGIC"]["DetalleIGIC"].append(
                ...
                if tax in taxes_sfess:
                    ...
                    if agency == 'aeat_sii_tax_agency_canary':                  
                        service_dict["Sujeta"].setdefault(
                            "NoExenta",
                            {"TipoNoExenta": "S1", "DesgloseIGIC": {"DetalleIGIC": []}},
                        )
        ...
        _logger.error("TAXES_DICT OUT: ")
        _logger.error(taxes_dict)
        _logger.error(not_in_amount_total)
    ...
    def _get_sii_in_taxes(self):
        ...
        for tax_line in tax_lines.values():
            ...
            if tax in taxes_sfrisp:
                ...
                if agency == 'aeat_sii_tax_agency_canary':
                    base_dict = taxes_dict.setdefault("DesgloseIGIC", {"DetalleIGIC": []})
                else:
            ...
            if tax in taxes_sfrns:
                if agency == 'aeat_sii_tax_agency_canary':
                    ...
                    base_dict["DetalleIGIC"].append(tax_dict)
                else:
                    tax_dict.pop("TipoImpositivo")
                    tax_dict.pop("CuotaSoportada")
    ...
    def _get_sii_invoice_dict_in(self, cancel=False):
        ...
        if cancel:
            ...
            if agency == 'aeat_sii_tax_agency_canary':
                for p in inv_dict["FacturaRecibida"]["DesgloseFactura"]["DesgloseIGIC"]["DetalleIGIC"]:
                    p.pop("CuotaSoportada")
                    _logger.error("EXTRAIDA CUOTA SOPORTADA")
    ...
    def _connect_params_sii(self, mapping_key):
        ...
        if self.sii_agency:
            if self.sii_agency in self._get_available_agencies():
                agency = self.sii_agency
            else:
                raise exceptions.UserError("Los impuestos seleccionados no pueden ser enviados a la agencia indicada")
        else:
            self.sii_agency = self._get_agency_id()
            agency = self.sii_agency

        if agency:
            params.update(agency._connect_params_sii(mapping_key, self.company_id))
        else:
            raise exceptions.UserError("Los impuestos incluidos en el documento, no pertenecen a ningún mapeo del SII.")
    ...
    def _connect_sii(self, mapping_key):
        ...
        if params["wsdl"]:
            _logger.error("WSDL: " + params["wsdl"] )
        if params["port_name"]:
            _logger.error(" PORT: " + params["port_name"])
        if params["address"]:
            _logger.error(" ADDRESS: " + params["address"])
    ...
    def _send_invoice_to_sii(self):
        for invoice in self.filtered(lambda i: i.state in SII_VALID_INVOICE_STATES):
            ...
            try:
                ...
                _logger.error("DATOS ENVIO:")
                _logger.error(inv_vals)
    ...
    def _get_agency_id(self):
        ...
        sii_maps = self.env["aeat.sii.map"].search([])

        #agency = []
        agency = set()

        for tax in self.line_ids.tax_ids: # PARA LOS IMPUESTOS DE LAS LINEAS DE LAS FACTURAS
            for map_line in sii_maps.map_lines: # PARA LAS LINEAS DE LOS MAPEOS
                for map_tax in map_line.taxes: # PARA EL IMPUESTO DE CADA LINEA DE LOS MAPEOS
                    map_line_tax_id = self.company_id._get_tax_id_from_tax_template(map_tax, self.company_id) #SE OBTIENE TEMPLATE DEL IMPUESTO

                    if map_line_tax_id and map_line_tax_id == tax.id and map_line.sii_map_id:
                        agencias = self.env["aeat.sii.tax.agency"].search(["&","&", ("id", "in", self.company_id.sii_tax_agency_id.ids), ("agency_aeat_sii_map", "=", map_line.sii_map_id.id), ("active", "=", True)])
                        for agencia in agencias:
                            #agency.append(agencia)
                            agency.add(agencia)
        _logger.info(next(iter(agency)))
        if len(agency) == 1:
            #return agency[0]
            return next(iter(agency))
        elif len(agency) == 0:
            raise exceptions.UserError("Los impuestos incluidos en el documento, no pertenecen a ningún mapeo del SII.")
        else:
            raise exceptions.UserError("Los impuestos incluidos en el documento no pueden pertenecer a diferentes agencias. Por favor, revise los impuestos y si son correctos, elija una agencia a la que enviar")


    def _get_available_agencies(self):
